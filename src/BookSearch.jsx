...
function AutocompleteSearch() {
  ...
  useEffect(() => {
    const filterSuggestions = () => {
      if (userInput) {
        const filteredSuggestions = data.summaries.filter((summary) =>
          summary.summary.toLowerCase().includes(userInput.toLowerCase())
        );

        filteredSuggestions.sort((a, b) => {
          const countA =
            a.summary.toLowerCase().split(userInput.toLowerCase()).length - 1;
          const countB =
            b.summary.toLowerCase().split(userInput.toLowerCase()).length - 1;
        ...
        setSuggestions(
          filteredSuggestions
            .slice(0, 6)
            .map((summary) => data.titles[summary.id])
        ); // Extract titles
      } else {
        setSuggestions([]);
      }
  ...
  const handleSuggestionClick = (suggestion) => {
    const matchingSummary = data.summaries.find(
      (summary) => data.titles[summary.id] === suggestion
    );
    console.log("matchingSummay", matchingSummary);
    const matchingAuthor = data.authors.find(
      (author) => author.book_id === matchingSummary.id
    ).author;
    console.log("matchingAuthor", matchingAuthor);
    setCards([
      ...cards,
      {
        title: suggestion,
